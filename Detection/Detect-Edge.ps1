<#
.SYNOPSIS
    Detects whether an application (Microsoft Edge) is installed and meets the expected version.
...
.EXAMPLE
    ...
    powershell -NoProfile -NonInteractive -File .\Detection\Detect-Edge.ps1 -ExpectedVersion 149.0.4022.62

.EXAMPLE
    # Enable local file logging
    powershell -NoProfile -NonInteractive -File .\Detection\Detect-Edge.ps1 -ExpectedVersion 149.0.4022.62 -EnableLogging $true
...
param (
    ...
    [string]$AppDisplayName = "Edge",
    [string[]]$MachinePaths = @(
        "${env:ProgramW6432}\Microsoft\Edge\Application\msedge.exe",
        "${env:ProgramW6432}\Microsoft\Edge\Application\new_msedge.exe",
    ...
    [string]$ExpectedVersion = "149.0.4022.62",
