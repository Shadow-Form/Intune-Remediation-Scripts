...
only occur when `-EnableLogging` is true or a `-LogFile` is provided.
...
Array of one-or-more file glob patterns (e.g. `%ProgramFiles%\TechSmith\Snagit\*.exe`) to
    ...
    Relative path(s) under each user profile to detect per-user installs (e.g.
        ...
        When `$true` the script may derive and write a local log file to the host `TEMP` directory
        if no `-LogFile` is provided. Keep detection read-only by omitting both
        parameters.
        ...
        .EXAMPLE
        powershell -NoProfile -NonInteractive -File .\Detection\Detect-Snagit.ps1 -ExpectedVersion 26.1.1 -EnableLogging $true -LogFile C:\Temp\Detect-Snagit.log -Verbose
