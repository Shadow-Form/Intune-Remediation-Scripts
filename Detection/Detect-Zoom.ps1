<#
.SYNOPSIS
    Detects whether an application (Zoom) is installed and meets the expected version.
...
.EXAMPLE
    # Check Zoom version (no file logging)
    powershell -NoProfile -NonInteractive -File .\Detection\Detect-Zoom.ps1 -ExpectedVersion 7.0.2.34412

.EXAMPLE
    # Enable local file logging
    powershell -NoProfile -NonInteractive -File .\Detection\Detect-Zoom.ps1 -ExpectedVersion 7.0.2.34412 -EnableLogging $true
...
param (
    ...
    [string]$AppDisplayName = "Zoom",
    [string[]]$MachinePaths = @(
        "${env:ProgramW6432}\Zoom\bin\Zoom.exe",
    ...
    [string]$ExpectedVersion = "7.0.2.34412",
