<#
.SYNOPSIS
    Detects whether an application (SAP GUI) is installed and meets the expected version.
...
param (
    ...
    [string]$AppDisplayName = "SAP GUI",
    [string[]]$MachinePaths = @(
        "${env:ProgramFiles(x86)}\SAP\FrontEnd\SAPGUI\SAPGUI.EXE"
    ),
    # Relate paths under each user profile; if empty or null, per-user search is skipped
    [string[]]$PerUserRelativePath = @(
        ""
    ),
    [string]$ExpectedVersion = "8000.1.13.8962",
