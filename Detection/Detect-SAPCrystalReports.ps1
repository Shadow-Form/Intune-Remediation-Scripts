<#
.SYNOPSIS
    Detects whether an application (SAP Crystal Reports) is installed and meets the expected version.
...
.EXAMPLE
    # Check SAP Crystal Reports version (no file logging)
    powershell -NoProfile -NonInteractive -File .\Detection\Detect-SAPCrystalReports.ps1 -ExpectedVersion 14.4.0.5707

.EXAMPLE
    # Enable local file logging
    powershell -NoProfile -NonInteractive -File .\Detection\Detect-SAPCrystalReports.ps1 -ExpectedVersion 14.4.0.5707 -EnableLogging $true
...
param (
    ...
    [string]$AppDisplayName = "SAPCrystalReports",
    [string[]]$MachinePaths = @(
        "${env:ProgramFiles(x86)}\SAP BusinessObjects\SAP BusinessObjects Enterprise XI 4.0\win64_x64\crw64.exe"
    ),
    # Relate paths under each user profile; if empty or null, per-user search is skipped
    [string[]]$PerUserRelativePath = @(
        ""
    ),
    [string]$ExpectedVersion = "14.4.0.5707",
