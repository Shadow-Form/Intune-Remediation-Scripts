<#
.SYNOPSIS
    Detects whether an application (SAPBIPLATCLNT) is installed and meets the expected version.
...
.EXAMPLE
    # Check SAPBIPLATCLNT version (no file logging)
    powershell -NoProfile -NonInteractive -File .\Detection\Detect-SAPBIPLATCLNT.ps1 -ExpectedVersion 14.4.0.5707

.EXAMPLE
    # Enable local file logging
    powershell -NoProfile -NonInteractive -File .\Detection\Detect-SAPBIPLATCLNT.ps1 -ExpectedVersion 14.4.0.5707 -EnableLogging $true
...
param (
    ...
    [string]$AppDisplayName = "SAPBIPLATCLNT",
    [string[]]$MachinePaths = @(
        "${env:ProgramFiles(x86)}\SAP BusinessObjects\setup.exe"
    ),
