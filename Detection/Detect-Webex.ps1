<#
.SYNOPSIS
    Detects whether an application (Cisco Webex) is installed and meets the expected version.
...
.EXAMPLE
    # Check Webex version (no file logging)
    powershell -NoProfile -NonInteractive -File .\Detection\Detect-Webex.ps1 -ExpectedVersion 46.4.0.34752

.EXAMPLE
    # Enable local file logging
    powershell -NoProfile -NonInteractive -File .\Detection\Detect-Webex.ps1 -ExpectedVersion 46.4.0.34752 -EnableLogging $true
...
param (
    ...
    [string]$AppDisplayName = "Webex",
    [string[]]$MachinePaths = @(
        "${env:ProgramW6432}\Cisco Spark\spark-windows-app.dll",
        "${env:ProgramFiles(x86)}\Cisco Spark\spark-windows-app.dll"
    ),
    # Relate paths under each user profile; if empty or null, per-user search is skipped
    [string[]]$PerUserRelativePath = @(
        "AppData\Local\Programs\Cisco Spark\spark-windows-app.dll"
    ),
    [string]$ExpectedVersion = "46.4.0.34752",
