<#
.SYNOPSIS
Ensure Google Chrome is installed (idempotent, non-interactive Intune remediation).
...
Display name used for logs, marker file names, and display matching. Default: 'Chrome'.
...
powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .\Remediation\Install-Chrome.ps1 -ExpectedVersion 147.0.7727.138

.EXAMPLE
# Remediate with file logging enabled
powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .\Remediation\Install-Chrome.ps1 -ExpectedVersion 147.0.7727.138 -EnableLogging
...
param (
    ...
    [string]$PerUserRelativeExePath = "AppData\Local\Google\Chrome\Application\chrome.exe",
    [ValidateNotNullOrEmpty()]
